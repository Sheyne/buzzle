...
export function ordered(players: readonly [string, string]) {
...
export class Game {
  constructor(
    ...
    private upcomingPairs: (readonly [string, string])[] = []
  ) {}

  public static fromJSON(state: StateJson) {
    return new Game(
      new Set(state.players),
      new Map(state.buzzles.map(([p1, p2, time]) => [[p1, p2], time])),
      state.upcoming
    );
  }
  ...
  public toJSON(): StateJson {
    return {
      ...
      buzzles: [...this.buzzles.entries()].map<[string, string, number]>(
        ([[p1, p2], time]) => [p1, p2, time]
      ),
      upcoming: this.upcomingPairs,
    };
  }
  ...
  public resetUpcoming(toValue: (readonly [string, string])[] = []) {
    this.upcomingPairs = toValue;
  }

  public addRound(players: readonly [string, string], time: number) {
    const orderedPlayers = ordered(players);
    this.buzzles.set(orderedPlayers, time);
    const nextPlayers = this.upcoming[0];
    if (
      nextPlayers &&
      nextPlayers[0] === players[0] &&
      nextPlayers[1] === players[1]
    ) {
      this.upcoming.shift();
    }
  }
  ...
  private untilValid(
    getPlayers: () => [string, string] | undefined
  ): [string, string] | undefined {
    for (let x = 0; x < 500; x += 1) {
      const choice = getPlayers();
      if (
        choice != null &&
        ![...this.buzzles.keys()].find(
          ([a, b]) => a === choice[0] && b === choice[1]
        )
      ) {
        return choice;
      }
  ...
  private findNextPairingInternal({ diameter }: { diameter: number }) {
    const counts = this.buzzleCounts().filter(
      ([player]) => this.players.has(player) && !this.isTooRecent(player)
    );
  ...
  public get upcoming(): (readonly [string, string])[] {
    this.calculateUpcoming();
    return this.upcomingPairs;
  }

  public leaderboard(limit?: number | null): [string, string, number][] {
    const times = [...this.buzzles.entries()].map<[string, string, number]>(
      ([[p1, p2], t]) => [p1, p2, t]
    );
